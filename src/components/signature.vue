    ...
    <el-dialog :title="dialogName" v-model="dialogVisible" width="600px" @close="colse">
        <div class="dialogInfo" v-loading="loading">
            <div class="marb_10">
                <div class="modify">
                    <p>{{
                        props.dialogName == "修改质押人地址" ? "现质押人地址" : props.dialogName == "修改收益人地址" ? "现收益人地址" : "钱包地址"
                        }}</p>
                ...
                <div class="modify">
                    ...
                    <ul>
                        <li v-for="(item, index) in currency" :key="index" @click="chooseCurrency(index)"
                            :class="{ selectedColor: currencyActive == index }"><img :src="item.icon"
                                                                                     alt=""/>{{ item.name }}
                        </li>
                    </ul>
                </div>
            ...
            <div class="line modify"
                 v-if="props.dialogName == '修改质押人地址' || props.dialogName == '修改收益人地址'">
                <p class="mar_30">{{
                    props.dialogName == "修改质押人地址" ? "新质押人地址" : props.dialogName == "修改收益人地址" ? "新收益人地址" : "钱包地址"
                    }}</p>
                <el-input class="mar_30" v-model="new_addr" placeholder="请输入钱包地址"
                          onkeyup="value=value.replace(/\s+/g, '')" style="width: 400px"></el-input>
            </div>
