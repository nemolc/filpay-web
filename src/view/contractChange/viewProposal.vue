<template>
  <div v-show="!isshow" v-loading="loading" class="minheight">
    <div class="card" v-for="(item, i) in proposals"
      ...
      <div class="btn" :class="item.stat == -1 ? 'underway' : item.stat == 0
        ? 'primary'
        : item.stat == 1
          ? 'success'
          : 'error'
        ">{{ proposalStatus(item) }}</div>
    </div>
  </div>
  <div class="container" v-show="isshow">
    <div class="formInfo">
      <div class="title">{{ proposalName(details) }}</div>
      <div class="btn" :class="details.stat == -1 ? 'underway' : details.stat == 0
        ? 'primary'
        : details.stat == 1
          ? 'success'
          : 'error'
...
const proposalsFun = async () => {
  ...
  try {
    const res = await get(
      `${sessionStorage.getItem("network")}/${Route.query.id}/proposals`
    );
