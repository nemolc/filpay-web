<template>
  <div class="formInfo">
    <el-form ref="target" :model="form" label-position="left" label-width="153px" :hide-required-asterisk="true">
      <el-form-item :label="'收益人' + (i + 1)" v-for="(item, i) in form.new_allot_ratios" :key="i"
        :prop="`new_allot_ratios[${i}].addr`" :rules="[{ required: true, message: '请填写收益人地址', trigger: 'blur' }]">
        <el-input style="width: 300px" v-model="item.addr" placeholder="请填写收益人地址"></el-input>
        <el-input style="width: 90px; margin: 0 10px" v-model="item.ratio" placeholder="0"  oninput="value=value.replace(/(^\.|[^\d\.])/g,'').replace('.','$#$').replace(/\./g,'').replace('$#$','.').replace(/^\d{3,}.*/,'100')"><template #suffix>%</template></el-input>
