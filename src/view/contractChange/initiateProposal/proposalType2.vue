<template>
  <div class="formInfo">
    <el-form ref="target" :model="form" :rules="rules" label-position="left" label-width="153px" :hide-required-asterisk="true">
      <el-form-item label="现收益起始释放高度">
        <el-input
          disabled
          v-model="details.beneficiary_release_height"
          placeholder="请输入区块高度"
          style="width: 180px"
        ></el-input>
      ...
      <el-form-item label="新收益起始释放高度" prop="new_beneficiary_release_height">
        <el-input
          v-model="form.new_beneficiary_release_height"
          placeholder="请输入区块高度" @blur="getTime(form.new_beneficiary_release_height, 2)"
          style="width: 180px"
        ></el-input>
        <img src="@/assets/change.png" alt="" />
        <el-date-picker
  ...
  <signature :dialogName="'变更产出收益释放高度'" :parentForm="form" v-model:show="visible"></signature>
</template>
  <script setup>
...
const form = reactive({
  date1:"",
  date2:"",
  "new_beneficiary_release_height": ""
});
...
onMounted(() => { 
  getTime(props.details.beneficiary_release_height,1)
});
...
const setHeight = () => {
  ...
  form.new_beneficiary_release_height=height
