<template>
  ...
  <div class="container">
    ...
    <div class="formInfo">
      <el-form :model="form" ref="target" :rules="rules" label-position="left" label-width="180px" class="ruleForm"
        ...
        <el-form-item label="" prop="new_quota">
          <el-input v-model="form.new_quota" placeholder="请输入提币额度上限" style="width: 400px">
            <template #suffix>
              <span>FIL</span>
            </template></el-input>
          <div class="tips">
        ...
        <el-form-item label="产出收益起始释放高度" prop="beneficiary_release_height">
          <el-input v-model="form.beneficiary_release_height" placeholder="请输入区块高度"
            @blur="getTime(form.beneficiary_release_height, 1)" style="width: 180px"></el-input>
          <img src="@/assets/change.png" alt="" />
          <el-date-picker v-model="form.date1" type="datetime" @change="setHeight(1)" placeholder="请输入授权截止期限"
            style="width: 180px">
          </el-date-picker>
          <div class="tips">
        ...
        <el-form-item label="质押退还起始释放高度" prop="invested_release_height">
          <el-input v-model="form.invested_release_height" placeholder="请输入区块高度"
            @blur="getTime(form.invested_release_height, 2)" style="width: 180px"></el-input>
          <img src="@/assets/change.png" alt="" />
          <el-date-picker v-model="form.date2" type="datetime" @change="setHeight(2)" placeholder="请输入授权截止期限"
            style="width: 180px">
          </el-date-picker>
          <div class="tips">
        ...
        <el-form-item :label="'收益人' + (i + 1)" v-for="(item, i) in form.beneficiarys_allot_ratios" :key="i"
          :prop="`beneficiarys_allot_ratios[${i}].addr`" :rules="[
            { required: true, message: '请填写收益人地址', trigger: 'blur' },
          ]">
          <el-input style="width: 300px" v-model="item.addr" placeholder="请填写收益人地址"></el-input>
          <el-input style="width: 90px; margin: 0 10px" v-model="item.ratio"
            oninput="value=value.replace(/(^\.|[^\d\.])/g,'').replace('.','$#$').replace(/\./g,'').replace('$#$','.').replace(/^\d{3,}.*/,'100')"
            placeholder="0"><template #suffix>
              %
            </template>
          </el-input>
        ...
        <el-form-item :label="'质押人' + (i + 1)" v-for="(item, i) in form.investors_allot_ratios" :key="i"
          :prop="`investors_allot_ratios[${i}].addr`" :rules="[
            { required: true, message: '请填写质押人地址', trigger: 'blur' },
          ]">
          <el-input style="width: 300px" v-model="item.addr" placeholder="请填写质押人地址"></el-input>
          <el-input style="width: 90px; margin: 0 10px" v-model="item.ratio" placeholder="0"
            oninput="value=value.replace(/(^\.|[^\d\.])/g,'').replace('.','$#$').replace(/\./g,'').replace('$#$','.').replace(/^\d{3,}.*/,'100')">
            <template #suffix>
              %
            </template></el-input>
...
const rules = reactive({
  invested_funds: [
    { required: true, message: "请输入总质押币", trigger: "blur" },
  ],
