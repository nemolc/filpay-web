<template>
	<div class="container">
		...
		<div class="formInfo">
			<el-form :hide-required-asterisk="true" ref="target" :model="form" :rules="rules" label-position="left" label-width="153px">
				<el-form-item label="提币数量" prop="amount">
					<el-input style="width: 400px" v-model="form.amount" placeholder="请输入提币数量"
						><template #suffix>
							<span>FIL</span>
						</template></el-input
					>
			...
			<div class="add">
				注：所有提币均默认充入节点Beneficiary合约地址，随即按合约比例自动分配给各收益人；<br />
				若提币发生在质押退还期，则优先按合约比例自动退还给各质押人，待退还累计额达到质押池总额后，<br />超出的部分恢复分配给各收益人。
			</div>
		</div>
